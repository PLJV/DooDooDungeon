...
curl --silent --output index.html https://www.faa.gov/air_traffic/flight_info/aeronav/digital_products/dof/ >> index.html >/dev/null 2>&1
...
rm -rf *DAT *EXE *exe;
7za a `ls -1 wind_turbines_*.shp | awk '{ gsub(".shp",".zip"); print }'` wind_turbines_* >/dev/null 2>&1
rm -rf `ls -1 wind_turbines_* | grep -v "zip"`;

if [ -d /gis_data/Wind/faa_products/ ]; then
  mv wind_turbines_*.zip /gis_data/Wind/faa_products/
fi
